 ...
 * 5. JavaScript format selection (Vite, Vanilla, or none)
 ...
 *   - js: string|false - JavaScript format ('vite', 'vanilla', or false)
...
async function prompting () {
   ...
   * - JavaScript format: Vite (bundled with imports) or Vanilla (direct reference)
  ...
  const remainingAnswers = await prompt([
    {
      ...
      message: 'JS format',
      choices: [
        { message: 'Vite', value: 'vite' },
        { message: 'Vanilla', value: 'vanilla' },
...
async function writing (answers) {
  ...
  const imports = [
    ...
    ...(answers.js === 'vite' ? [`${tag}.behavior.js`] : [])
  ]
  ...
  const exports = {
    ...
    ...(answers.js === 'vite' && { script: `../../dist/${tag}.js` }),
    ...(answers.js === 'vanilla' && { script: `${tag}.behavior.js` })
  }
  ...
  const templates = [
    ...
    'twig',
    'stories.js',
    answers.style || answers.js === 'vite' ? 'library.js' : null,
